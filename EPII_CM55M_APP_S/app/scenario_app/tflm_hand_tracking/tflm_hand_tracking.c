/*
 * tflm_hand_tracking.c
 *
 * Hand Tracking Application for Grove Vision AI V2
 * Based on tflm_fd_fm template
 */
...
struct_hand_algoResult algoresult_hand;
...
static void dp_app_hand_tracking_eventhdl_cb(EVT_INDEX_E event)
{
	uint16_t err;
	int32_t read_status;
	...
	if( g_trans_type == 0 )// transfer type is (UART)
	{
		cv_hand_tracking_run(&algoresult_hand);
#ifdef CIS_IMX
		hx_drv_scu_get_version(&chipid, &version);
		if (chipid == WE2_CHIP_VERSION_C)   // mipi workaround for WE2 chip version C
		{
	...
	else if( g_trans_type == 1 || g_trans_type == 2)// transfer type is (SPI) or (UART & SPI) 
	{
		...
		#if FRAME_CHECK_DEBUG
				hx_drv_spi_mst_protocol_write_sp((uint32_t)&algoresult_hand, sizeof(struct_hand_algoResult), DATA_TYPE_META_DATA);
		#endif
	}
#else
	#if TOTAL_STEP_TICK
		...
		#if TOTAL_STEP_TICK_DBG_LOG
				xprintf("Tick for TOTAL HAND_TRACKING:[%d]\r\n",(loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2));
	...
	#endif
#endif
		//clear hand tracking result
		algoresult_hand.num_hands = 0;
		for (int h = 0; h < 2; h++) {
			algoresult_hand.hands[h].detected = false;
			algoresult_hand.hands[h].palm.x = 0;
			algoresult_hand.hands[h].palm.y = 0;
			algoresult_hand.hands[h].palm.width = 0;
			algoresult_hand.hands[h].palm.height = 0;
			algoresult_hand.hands[h].palm.score = 0;
			algoresult_hand.hands[h].palm.rotation = 0;
			algoresult_hand.hands[h].handedness = 0;
			algoresult_hand.hands[h].score = 0;
			for (int i = 0; i < 21; i++) {
				algoresult_hand.hands[h].landmarks[i].x = 0;
				algoresult_hand.hands[h].landmarks[i].y = 0;
				algoresult_hand.hands[h].landmarks[i].z = 0;
			}
		}
	}
...
void app_start_state(APP_STATE_E state)
{
	...
	{
		if (state == APP_STATE_HAND_TRACKING)
		{
			if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_hand_tracking_eventhdl_cb, 4, APP_DP_RES_YUV640x480_INP_SUBSAMPLE_2X) < 0)// YUV 640X480
			{
				xprintf("\r\nDATAPATH Init fail\r\n");
				APP_BLOCK_FUNC();
			}
	...
	else if ( g_trans_type == 1 ) // only use SPI 
	{
		if(state == APP_STATE_HAND_TRACKING) {
			...
			if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_hand_tracking_eventhdl_cb, 4, APP_DP_RES_YUV640x480_INP_SUBSAMPLE_1X) < 0)// YUV 640X480
			{
				xprintf("\r\nDATAPATH Init fail\r\n");
				APP_BLOCK_FUNC();
			}
...
int app_main(void) {
	...
	//Hand Tracking
	xprintf("Hand Tracking Init\n");
	cv_hand_tracking_init(true, true, PALM_DETECT_FLASH_ADDR, HAND_LANDMARK_FLASH_ADDR);
	app_start_state(APP_STATE_HAND_TRACKING);
